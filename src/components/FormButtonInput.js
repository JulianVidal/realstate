...
class FormButtonInput extends Component {
  ...
  render() {
    return (
      <div
        className={
          "FormButtonInput" +
          (this.props.text.includes("Google") ? " google" : "")
        }
        onClick={() => this.handleSubmit(this.props.submit)}
      >
        {this.props.icon}
  ...
  handleSubmit = async (submitFunc) => {
    ...
    iconLoading.querySelector("path").style.stroke = this.props.text.includes(
      "Google"
    )
      ? "#212121"
      : "#FAFAFA";
    ...
    if (
      iconCheckMark.style.opacity === "1" ||
      iconAlert.style.opacity === "1" ||
      iconLoading.style.opacity === "1"
    )
      return;
    if (
    ...
    tl.to(container, { duration: 0.4, width: "45px", ease: Power3.easeOut }) // Width of input to 45px
      .to(
        this.submitButton,
        { duration: 0.1, borderRadius: "45px", ease: Power3.easeOut },
        0.1
      ) // Turns input into a circle
    ...
    if (!isGoogle) {
      tl.to(
        iconLoading,
        {
          ...
          ease: Power3.easeOut,
          onEnterFrame: () => {
            this.setState({ loadingIsStopped: false });
          },
        },
        0.23
      ); // Opacity 1 for Loading Icon
    }

    await tl
      .call(async () => {
        tl.pause();
        error = await submitFunc();
        tl.resume();
      }) // Waits until function is done
      .to(iconLoading, { duration: 0.23, opacity: 0, ease: Power3.easeOut }); // Opacity 0 for Loading Icon

    if (error) {
      const type = error.code.includes("email")
        ? ".email"
      ...
      tl.call(() => {
        console.log("Unsuccesful request for sign up or log into server");
      });
      if (!isGoogle) {
        tl.to(iconAlert, {
          duration: 0.23,
          ...
          ease: Power3.easeOut,
          onEnterFrame: () => {
            this.setState({ alertIsStopped: false });
            iconLoading.style.display = "none";
            this.setState({ loadingIsStopped: true });
          },
        });
      }
      ...
      tl.to(
        ".error-message",
        { duration: 0.23, height: "0", ease: Power3.easeOut },
        "-=0.23"
      )
        .to(
          this.submitButton,
          { duration: 0.23, backgroundColor: "#D8000C", ease: Power3.easeOut },
          "-=0.23"
        )
        .to(
          ".error-block" + type,
        ...
        .to(iconAlert, {
          ...
          ease: Power3.easeOut,
          onComplete: () => {
            this.setState({ alertIsStopped: true });
            iconAlert.style.display = "none";
          },
        })
        .to(
          this.submitButton,
          { duration: 0.23, backgroundColor: bgColor, ease: Power3.easeOut },
          "-=0.23"
        )
        .to(container, { duration: 0.4, width: "100%", ease: Power3.easeOut })
        .to(
          this.submitButton,
          { duration: 0.4, borderRadius: "8px", ease: Power3.easeOut },
          "-=0.4"
        )
        .to(
          this.submitButton,
          { duration: 0.23, color: fgColor, ease: Power3.easeOut },
          "-=0.4"
        )
        .to(".error-block", {
          duration: 0.23,
        ...
        .to(
          ".error-message" + type,
          { duration: 0.23, height: "13px", ease: Power3.easeOut },
          "-=0.22"
        );
    } else {
      tl.call(() => {
        console.log("Succesful request for sign up or login into server");
      });
      if (!isGoogle) {
        tl.to(iconCheckMark, {
          duration: 0.23,
          ...
          ease: Power3.easeOut,
          onEnterFrame: () => {
            this.setState({ checkmarkIsStopped: false });
            iconLoading.style.display = "none";
            this.setState({ loadingIsStopped: true });
          },
        }); // opacity 1 for check mark and removes loading icon
      }

      tl.to(
        this.submitButton,
        { duration: 0.23, backgroundColor: "#00C853", ease: Power3.easeOut },
        "-=0.23"
      ) // Changes background color to green
        .to("#NavLogIn, #NavSignUp", {
          duration: 0.23,
          opacity: 0,
          ease: Power3.easeOut,
        })
        ...
        .set(iconCheckMark, {
          opacity: 0,
          onComplete: () => {
            this.setState({ checkmarkIsStopped: true });
